...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    def eval[F[_],A](a: F[A]): Process[F,A] = await(a)( (ei:Either[Throwable, A]) => ei match {
        case Left(err) => Halt(err)
        case Right(a:A) => Emit(a, Halt(Kill))
    }
    )

    /* Evaluate the action purely for its effects. */
    def eval_[F[_],A,B](a: F[A]): Process[F,B] = eval[F,A](a).drain[B] //drain exhausts the whole input without emiting
    ...
    def join[F[_],A](p: Process[F,Process[F,A]]): Process[F,A] = p flatMap(x=>x)
